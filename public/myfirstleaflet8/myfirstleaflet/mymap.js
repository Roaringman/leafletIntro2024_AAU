...
let getData = async function () {
  const geodata = await fetch(
    "http://localhost:3000/myfirstleaflet8/myfirstleaflet/counties.geojson"
  );
...
const createMap = function () {
  ...
  let controls = L.control.layers(basemaps, overlays).addTo(mymap);
  ...
  getData().then((countyData) => {
    ...
    controls.addOverlay(geojsonCountyData, "Counties");
  });
};
createMap();
